...
<meta charset="ISO-8859-1" />
	...
	<style>
			body{
			background-image: url('imgs/inq.jpg');
...
function Function3() {
  ...
  <body onload="startTime(); visibility();">
   ...
   <div class="login-box">
        ...
        <form name="formal" method="post" action="ViewIn" autocomplete="off" >
	...
	<c:forEach var="in" items="${inquiryDetails}"> 
			...
			<c:set var="id" value="${in.id}"/>
			<c:set var="name" value="${in.name}"/>
			<c:set var="email" value="${in.email}"/>
			<c:set var="number" value="${in.number}"/>
			<c:set var="faculty" value="${in.faculty}"/>
			<c:set var="year" value="${in.year}"/>
			<c:set var="inquirytype" value="${in.inquirytype}"/>
			<c:set var="message" value="${in.message}"/>
	...
	<br><br><br>
		<center>
		 	<table style=" border-radius: 10px; padding:20px; margin-bottom: 50px; width:60% ; background:#87CEEB; box-shadow: 4px 2px 20px ;">
		 		<tr>
		 			<td>
						<h3>Name   :  ${in.name} <br/>
						...
						Year      :  ${in.year} <br/><br>
						Inquiry Type   :  ${in.inquirytype} <br/><br>
						Message   :  ${in.message} <br/>
						
						</h3>
						<br>
						
						<center><img src="inquiry/${in.filename}" width="60%" height="300px"/></center>
					</td>
				</tr>
			</table>
		</center>
	   		
			<a href="${inquiryUpdate}">
